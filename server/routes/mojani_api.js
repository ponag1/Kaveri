...
router.get('/api/getLandRecordsMojaniByPid/:id', (req, res) => {
	...
	if(!isNaN(req.params.id)){
		requestify.get('http://ec2-52-206-196-6.compute-1.amazonaws.com:3001/api/org.bhoomi.landrecords.LandRecord/'+req.params.id).then(function(response) {
			land_details = response.getBody();
			owner = land_details.owner;
			owner = owner.split("#")[1];
			console.log("Land details bC: "+JSON.stringify(land_details));
			requestify.get('http://ec2-52-206-196-6.compute-1.amazonaws.com:3001/api/org.bhoomi.landrecords.Assignee/'+owner).then(function(response) {
